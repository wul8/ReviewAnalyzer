...
class AeProcessor(DataProcessor):
    ...
    # Further modificaiton
    def get_input_examples(self, sentences):
        examples = []
        for index in range(len(sentences)):
            guid = "%s-%s" % ("input", index)
...
class AscProcessor(DataProcessor):
    ...
    def get_input_examples(self, text, aspects):
        examples = []
        for asp_idx in range(len(aspects)):
            guid = "%s-%s" % ("input", str(asp_idx))
            text_a = aspects[asp_idx][0]
            text_b = text
            label = "positive"
            examples.append(
...
def convert_sentences_to_features(examples, label_list, max_seq_length, tokenizer, mode):
    """Loads a data file into a list of `InputBatch`s."""  # check later if we can merge this function with the SQuAD preprocessing
